...
<template>
	<div :style="{backgroundImage:'url('+RegisterBGURL+')',height:864+'px',weight:1920+'px'}">
		<div class="registerFrom">
			<Card :bordered="false" dis-hover>
				<p slot="title">欢迎加入我们，开启您的阅读之旅 ^_^</p>
				<Input prefix="md-contact" name="idinput" type="text" placeholder="请输入您的账号" style="width: auto" 
					...
					<Tooltip transfer :max-width="200" class="ascyicon" v-show="veeErrors.has('idinput')" slot="suffix">
							<p slot="content">{{ veeErrors.first('idinput') }}</p>
							<Icon color="#ed4014" type="md-close-circle" />
					</Tooltip>
				</Input>
				<br/><br/>

				<Input prefix="ios-key-outline" name="pwinput" type="password" placeholder="请输入您的密码" style="width: auto"
				...
				<Input prefix="ios-key" name="repwinput" type="password" placeholder="请再次输入您的密码" style="width: auto"
					v-validate="{'required': 'true', 'is': pw}" v-model="repw" @keydown.enter.native="submit">
					<Tooltip transfer :max-width="200" class="ascyicon" v-show="veeErrors.has('repwinput')" slot="suffix">
				...
				<Input prefix="md-mail" name="emailinput" type="email" placeholder="请输入您的邮箱" style="width: auto"
				...
				<br/><br/>
				<font color="#808695" size="1">已有账号？立即<router-link to="/login">登录</router-link>!</font>
			</Card>
		</div>
		<BackTop :height="0" :bottom="30"  on-click="goToWelcomePage">
			<div class="register-back-to-welcome-top">返回首页</div>
		</BackTop>
	</div>
</template>
<script>
	...
	import RegisterBG from '../../pics/register_bg.jpg';
			...
			submit:function(){
				this.$validator.validate().then(result => {
					if (!result) {
						...
						// 采用字符串方式发送
						axios.post(Store.state.server+"/RegisterServlet","jsondata="+JSON.stringify(jsonMsg))
